...
class V1AccountValidationControllerTest(
    ...
    val v1TransactionCacheRepository = dependencies.v1TransactionCacheRepository
    ...
    given("외화 계좌 한도 확인 API 요청되어") {
            ...
            then("'500 Internal server error - 210_1002_005' 에러 응답 정상 확인한다") {
                result.andExpect {
                    ...
                    content {
                        ...
                        jsonPath("result.detailMessage", equalTo("balance: 0 < (pendingAmount: 0 + amount: 10000)"))
                    }
                }
            }
        }
        ...
        // 출금 합계 Cache 저장
        v1TransactionCacheRepository.incrementWithdrawalTransactionPendingAmountCache(account.acquirer.id, account.acquirer.type, 10001)

        `when`("외화 계좌 출금 합계 한도 초과인 경우") {
            ...
            then("'500 Internal server error - 210_1002_005' 에러 응답 정상 확인한다") {
                result.andExpect {
                    ...
                    content {
                        ...
                        jsonPath("result.code", equalTo("210_1002_005"))
                        jsonPath("result.message", equalTo("Account validation service failed. Account balance is insufficient."))
                        jsonPath("result.detailMessage", equalTo("balance: 20000 < (pendingAmount: 10001 + amount: 10000)"))
                    }
                }
            }
        }
        ...
        // 출금 합계 Cache 초기화
        v1TransactionCacheRepository.clearWithdrawalTransactionPendingAmountCache(account.acquirer.id, account.acquirer.type)
