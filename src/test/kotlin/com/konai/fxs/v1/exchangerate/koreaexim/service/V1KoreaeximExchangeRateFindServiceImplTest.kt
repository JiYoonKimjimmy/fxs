...
class V1KoreaeximExchangeRateFindServiceImplTest : CustomBehaviorSpec({
    ...
    given("한국수출입은행 환율 정보 조회 요청되어") {
        ...
        `when`("요청 정보 기준 조회 결과 없는 경우") {
            val exception = shouldThrow<ResourceNotFoundException> { v1KoreaeximExchangeRateFindService.findLatestExchangeRate(currency, requestDate) }
        ...
        `when`("요청 'currency' 기준 환율 Cache 정보 없는 경우") {
            val result = v1KoreaeximExchangeRateFindService.findLatestExchangeRate(currency, requestDate)
            ...
            then("DB 조회 결과 기준 Cache 정보 변경 정상 확인한다") {
                val cache = v1KoreaeximExchangeRateCacheRepository.findKoreaeximExchangeRateCache(currency)
                cache!! shouldNotBe null
                cache.curUnit shouldBe currency
                cache.registerDate shouldBe requestDate
                cache.ttb shouldBe "1000"
            }
        }
