...
class ExchangeRateCollectTimerSchedulerTest : CustomBehaviorSpec({
    ...
    listeners(
        ...
        MockRabbitMQTestListener(V1_EXCHANGE_RATE_COLLECTOR_TIMER_EXCHANGE)
    )
        ...
        `when`("Timer 메시지 발행 성공인 경우") {
            ...
            then("메지시 발행 건수 정상 확인한다") {
                repeat(koreaeximCollectorSize) {
                    rabbitTemplate.receiveAndConvert(V1_EXCHANGE_RATE_COLLECTOR_TIMER_EXCHANGE.queueName) shouldNotBe null
                }
            }
        }
    }
