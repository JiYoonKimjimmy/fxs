...
class TransactionCacheTypeTest : CustomStringSpec({
    ...
    "'acquirer' 기준 출금 준비 거래 Cache Key 생성 정상 확인한다" {
        ...
        // when
        val result = PREPARED_WITHDRAWAL_TRANSACTION_CACHE.getKey(trReferenceId, channel)

        // then
        result shouldBe "fxs:$trReferenceId:$channel:prepared:withdrawal:transaction"
    }
